...
## File management
run_dir = './'
fname_wt_input = os.path.join(run_dir, f'outputs.{istep-1}', f'NREL-2p4-107-step{istep-1}.yaml')
...
if rank == 0:
    ...
    aopt['general']['fname_output'] = f'NREL-2p4-107-step{istep}'
   ...
   #aopt['constraints']['tower']['frequency_1']['upper_bound'] = 0.315 # 10% below 3P cut-in, minOmega=7RPM
    ...
    mopt = load_yaml(os.path.join(run_dir,
                                  f'outputs.{istep-1}',
                                  f'NREL-2p4-107-step{istep-1}-modeling.yaml'))
    ...
    pklfile = os.path.join(run_dir, f'outputs.{istep-1}', f'NREL-2p4-107-step{istep-1}.pkl')
