...
## File management
run_dir = './'
fname_wt_input = os.path.join(run_dir, f'outputs.{istep-1}', f'NREL-2p3-107-step{istep-1}.yaml')
...
if rank == 0:
    ...
    aopt['general']['fname_output'] = f'NREL-2p3-107-step{istep}'
    ...
    mopt = load_yaml(os.path.join(run_dir,
                                  f'outputs.{istep-1}',
                                  f'NREL-2p3-107-step{istep-1}-modeling.yaml'))
