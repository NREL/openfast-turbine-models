...
def run_linearization():
    ...
    main_file        = 'NREL-2p8-127.fst'  # Main file in ref_dir, used as a template
    ...
    #op = pd.read_csv('../NREL-2.8-127_openfast.csv').set_index('Wind1VelX_[m/s]')
    op = pd.read_csv('../NREL-2.8-127_openfast.csv').set_index('rotor speed [RPM]')
